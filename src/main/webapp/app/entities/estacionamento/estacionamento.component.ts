...
currentAccount: any;
    ...
    loadAll() {
        this.estacionamentoService.query({
            page: this.page - 1,
    ...
    transition() {
        this.router.navigate(['/estacionamento'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Estacionamento) {
        return item.id;
    }
    registerChangeInEstacionamentos() {
        this.eventSubscriber = this.eventManager.subscribe('estacionamentoListModification', (response) => this.loadAll());
    }
